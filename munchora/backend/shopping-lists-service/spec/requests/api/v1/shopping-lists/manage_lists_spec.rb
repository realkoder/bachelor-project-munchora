...
RSpec.describe 'Manage shopping lists', type: :request do
  ...
  describe 'As a user I want to manage my shopping lists' do
    ...
    describe 'updating a shopping list' do
      let(:owner) { create(:shopping_list_owner, id: 1, auth_user_id: 1) }
      let!(:list) { create(:shopping_list, owner: owner) }

      it 'successfully with valid data' do
        expect(ShoppingLists::NotifyEvents).to receive(:name_updated).with(list)

        cookies[:jwt_auth] = token
        put "/api/v1/shopping_lists/#{list.id}", params: { shopping_list: { name: 'Misc' } }

        expect(response).to have_http_status(:ok)
        parsed_list = JSON.parse(response.body, symbolize_names: true)
        expect(parsed_list[:name]).to eq('Misc')
      end

      it 'fails without required fields' do
        cookies[:jwt_auth] = token
        put "/api/v1/shopping_lists/#{list.id}", params: { shopping_list: { name: '' } }

        expect(response).to have_http_status(:unprocessable_content)
      end

      it 'requires authentication' do
        put "/api/v1/shopping_lists/#{list.id}", params: { shopping_list: { name: 'Test' } }
    ...
    describe 'deleting a shopping list' do
      ...
      it 'rejects non owners from deleting list but will unshare' do
        ...
        expect(ShoppingLists::NotifyEvents).to receive(:unshare_list).with(no_access_list, user.id)
        expect(ShoppingLists::Sharer).to receive(:unshare).with(no_access_list, user.id)

        cookies[:jwt_auth] = token
        delete "/api/v1/shopping_lists/#{no_access_list.id}"

...
export default function ShoppingListDetail() {
  ...
  useEffect(() => {
    if (listId && shoppingLists && shoppingLists.length > 0) {
      const selectedList = shoppingLists.find((list) => list.id === listId);
      setCurShoppingList(selectedList);
    }
  }, [shoppingLists]);
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
              ...
              {isEditing ? (
                ...
                <div>
                  <div className="flex items-center space-x-3 mb-2">
                    <h1 className="text-3xl font-bold">{curShoppingList?.name}</h1>
                    <Badge
                      variant={curShoppingList && curShoppingList.shared_users.length > 0 ? 'default' : 'secondary'}
                      className={curShoppingList && curShoppingList.shared_users.length > 0 ? 'bg-blue-500' : ''}
                    >
        ...
        <div className="space-y-4">
          {CATEGORIES.filter((category) => curShoppingList?.items.some((item) => item.category === category)).map((category) => (
            <Card key={category as string}>
              <CardHeader>
                <CardTitle className="text-lg text-left">{category as string}</CardTitle>
                <CardDescription className="text-left">
                  {curShoppingList?.items.filter((item) => item.category === category && item.is_completed).length} of{' '}
                  {curShoppingList?.items.filter((item) => item.category === category).length} completed
                </CardDescription>
              </CardHeader>
              <CardContent>
          ...
          {/* Uncategorized items */}
          {curShoppingList && curShoppingList.items.filter((item) => !item.category).length > 0 && (
            <Card>
              <CardHeader>
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between text-sm mb-2">
                  ...
                  <span>
                    {curShoppingList?.items.filter((item) => item.is_completed).length} of {curShoppingList?.items.length}
                  </span>
                </div>
                {/* Progress */}
                {curShoppingList && curShoppingList?.items.length > 0 && (
                  <div>
                    <div className="flex justify-between text-sm text-gray-600 mb-2">
                      <span>Progress</span>
                      <span>
                        {curShoppingList.items.filter((item) => item.is_completed).length} of {curShoppingList.items.length} completed
                      </span>
                    </div>
                    <div className="w-full bg-third rounded-full h-2">
                      <div
                        ...
                        style={{
                          width: `${(curShoppingList.items.filter((item) => item.is_completed).length / curShoppingList.items.length) * 100}%`,
                        }}
                      ></div>
                    </div>
                  </div>
              ...
              <div className="flex justify-center space-x-4 pt-4">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-muted-foreground">
                    {curShoppingList ? curShoppingList.items.length - curShoppingList.items.filter((item) => item.is_completed).length : 0}
                  </div>

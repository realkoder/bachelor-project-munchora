...
import { CATEGORIES, type IShoppingList, type TCategory } from '~/types/shoppingList.interface';
...
interface ShoppingListProps {
  list: IShoppingList;
  setListToShare: (list: IShoppingList) => void;
  setShowShareModal: (value: boolean) => void;
}

export const ShoppingList = ({ list, setListToShare, setShowShareModal }: ShoppingListProps) => {
  const { updateListName, addItemToList, deleteShoppingListItem, toggleItemCompleted, updateItem, isAddingItem } = useShoppingLists();
  ...
  const handleShareClick = (list: IShoppingList) => {
    setListToShare(list);
        ...
        <div className="flex justify-between items-start">
          <div className="text-left">
            <CardTitle className="flex items-center space-x-2">
              ...
              <Button variant="outline" size="sm" onClick={() => navigate(`/shopping-list/${list.id}`)}>
          ...
          <div className="flex items-center space-x-2">
            ...
            <DeleteShoppingListDialog isOwner={list.owner_id === curUser?.user?.id} shoppingListId={list.id} />
          </div>
        </div>
      </CardHeader>
          ...
          {list.items
            ...
            .map((item) => (
              <div key={item.id} cy-data="list-item" className="flex items-center p-2 border-b border-secondary">
                ...
                <DeleteListItem item={item} handleDeleteItem={() => deleteShoppingListItem(list.id, item.id)} />
              </div>
            ))}
